...
interface CartItemRowProps {
  ...
  /** Product slug — when present, the image and name link to the product page. */
  productSlug?: string;
...
export function CartItemRow({ item, productSlug, onUpdate, onRemove }: CartItemRowProps) {
  ...
  const href = productSlug ? (`/product/${productSlug}` as Route) : null;

  const imageContent = item.productImageUrl ? (
    <Image
  ...
  const imageClassName = "relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100";

  return (
    <li className="flex gap-4 border-b py-4 last:border-b-0">
      {/* Product image */}
      {href ? (
        <Link href={href} className={imageClassName} aria-label={item.productName}>
          {imageContent}
        </Link>
      ) : (
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        {href ? (
          <Link
            href={href}
            className="truncate font-medium text-gray-900 transition-colors hover:text-amber-600"
          >
            {item.productName}
          </Link>
        ) : (
        ...
        <p className="text-sm text-gray-600">{formatPrice(item.unitPrice.amount)} / шт.</p>
      ...
      <div className="flex flex-shrink-0 flex-col items-end justify-between">
        <p className="font-semibold text-gray-900">{formatPrice(item.lineTotal.amount)}</p>
        <Button
          type="button"
          variant="ghost"
          size="icon"
